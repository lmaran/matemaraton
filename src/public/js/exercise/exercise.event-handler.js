...
export const eventHandlers = {
    toggleAnswer: async (event) => {
        ...
        event.target.textContent = divIsHide
            ? "Rezultat final"
            : "Ascunde rezultatul";
    },
    ...
    showNextHint: async () => {
        ...
        if (hintNr === totalHints) {
            document
                .getElementById("show-next-hint-btn")
                .classList.add("d-none");
        }
    },

    submitMySolution: async (event) => {
        const exerciseIdContainer = document.getElementById(
            "exercise-id-container"
        );
        ...
        const fileContainers = dropArea.querySelectorAll(
            ".file-container-span"
        );
        const files = [];
