...
exports.getPrettyJson = (object) => {
    ...
    // In the .hbs file, it requires <pre>, <code> and 2 curly braces: "<pre><code>{{objectAsJson}}</code></pre>""
    const objectAsJson = JSON.stringify(object, null, 4);

    // 2. highlight (inject html tags in order to support colors, borders etc)
    // In the .hbs file, it requires <pre>, <code> and 3 curly braces: "<pre><code>{{{prettyJson}}}</code></pre>""
